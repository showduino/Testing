...
void send_heartbeat_to_sue() {
  if (millis() - g_last_hb_ms < 500) return;
  g_last_hb_ms = millis();
  StaticJsonDocument<128> hb;
  hb["role"] = "IAN";
  hb["node"] = SHOWDUINO_NODE_ID;
  hb["flash_ok"] = g_flash_ok;
  char json[160];
  size_t n = serializeJson(hb, json, sizeof(json));
  PacketHeader hdr = make_hdr(MsgType::HEARTBEAT, SHOWDUINO_NODEID_SUE);
  g_sue_link.sendPacket(hdr, reinterpret_cast<const uint8_t *>(json), n);
}
