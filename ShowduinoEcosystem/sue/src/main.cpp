...
bool probe_i2c_addr(TwoWire &w, uint8_t addr) {
  w.beginTransmission(addr);
...
// --------------------
// SUE FILE SERVICE (subset fits ESP-NOW; full service is implemented but chunk size must be small)
// --------------------
void handle_file_op(const PacketView &pkt, const JsonDocument &req) {
  ...
  if (strcmp(op, "LIST") == 0) {
    ...
    uint8_t count = 0;
    while (f && count < 10) {  // keep small for ESP-NOW
  ...
  // Remaining ops exist but are usually too large for ESP-NOW in production.
...
void handle_from_ui(const uint8_t *mac, const uint8_t *data, int len, void *) {
  // Save UI mac and ensure it's a peer so we can respond.
  ...
  if (t == MsgType::COMMAND || t == MsgType::FILE_OP) {
    StaticJsonDocument<384> req;
    DeserializationError err = deserializeJson(req, pkt.payload, pkt.hdr.payload_len);
    if (err) {
    ...
    if (strcmp(cmd, "estop") == 0) {
      // Show-safe: set a fault + forward to all IANs (broadcast on UART, future: multi-IAN registry)
...
#if defined(SHOWDUINO_ENABLE_UART_IAN)
      // Forward command as-is to IAN (dst = broadcast)
    ...
    // Anything else: ACK now; full routing/orchestration expands here.
    StaticJsonDocument<128> ack;
    ack["ok"] = true;
    ack["note"] = "accepted";
    send_to_ui_json(MsgType::ACK, pkt.hdr.id, ack);
...
void handle_from_ian_uart(const PacketView &pkt, void *) {
  // Route IAN->UI messages (status/inventory) upward
...
void setup() {
  ...
  i2c_scan_print(Wire);
  g_rtc_ok = probe_i2c_addr(Wire, 0x68);
  g_aht_ok = probe_i2c_addr(Wire, 0x38);
...
void loop() {
  ...
  // PWM demo: keep off; future env automation owns this.
  delay(5);
}
